...
export function AdminDashboard() {
  ...
  const handleEdit = async (svg) => {
    ...
    if (svg.material && Array.isArray(svg.material)) {
      setMaterials(svg.material.map(m => m.en || m));
    } else {
      setMaterials([]);
    }
    setMaterialInput('');
  ...
  const handleUpdate = async (e) => {
    ...
    const updatedSvgData = {
      ...
      material: materials.map(material => ({ en: material }))
    };
                  ...
                  <div className="space-y-2">
                    <div className="flex space-x-2">
                      <input
                        ...
                        className="flex-1 px-3 py-2 border border-gray-300 rounded-sm shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
