  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        // The response.data contains the orders array
        ...
        const transformedOrders = ordersData.map(order => ({
          ...
          customer: {
            ...
            email: order.billingDetails?.email || order.customer?.email || 'No email',
            phone: order.billingDetails?.phone || order.customer?.phone || 'No phone'
          },
          shippingAddress: {
            name: order.shippingDetails?.firstName && order.shippingDetails?.lastName
              ? `${order.shippingDetails.firstName} ${order.shippingDetails.lastName}`
              : order.shippingAddress?.name || 'Unknown',
            street: order.shippingDetails?.streetAddress1 || order.shippingAddress?.street || 'No address',
            city: order.shippingDetails?.city || order.shippingAddress?.city || 'No city',
            state: order.shippingDetails?.state || order.shippingAddress?.state || 'No state',
            zip: order.shippingDetails?.zipCode || order.shippingAddress?.zip || 'No zip',
            country: order.shippingDetails?.country || order.shippingAddress?.country || 'No country'
          },
          paymentMethod: order.paymentDetails?.paymentMethod || order.paymentMethod || 'Check',
          transactionId: order.paymentDetails?.transactionId || order.transactionId || 'N/A',
