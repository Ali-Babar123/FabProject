...
export const uploadSvg = async (req, res) => {
  try {
    ...
    // Handle translations for title and description using backend translation
    const translatedTitle = await translateObject({ en: title.en || title });
    const translatedDescription = await translateObject({ en: description.en || description });

    // Handle material translations if provided
    const translatedMaterial = material && material.length > 0 
      ? await translateArray(material.map(item => ({ en: item.en || item })))
      : [];

    const translatedMaterialDescription = materialDescription 
      ? await translateObject({ en: materialDescription.en || materialDescription })
      : { en: '', es: '', fr: '', de: '' };
